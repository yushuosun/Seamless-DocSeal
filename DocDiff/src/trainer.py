...
class Trainer:
    ...
    def test(self):
        ...
        with torch.no_grad():
            self.network.init_predictor.load_state_dict(torch.load(self.TEST_INITIAL_PREDICTOR_WEIGHT_PATH, map_location=self.device))
            self.network.denoiser.load_state_dict(torch.load(self.TEST_DENOISER_WEIGHT_PATH, map_location=self.device))
            ...
            for img, gt, name in tq:
                ...
                if self.DPM_SOLVER == 'True':
                    sampledImgs = dpm_solver(
                        ...
                        steps=self.DPM_STEP,
                        cond=init_predict,
                        pre_ori=self.pre_ori,
                    )
                else:
                    sampledImgs = sampler(noisyImage, init_predict, self.pre_ori)
                ...
                img_save = torch.cat(
                    (
                        img.detach().cpu(),
                        gt.detach().cpu(),
                        init_predict.detach().cpu(),
                        min_max(sampledImgs.detach().cpu()),
                        finalImgs.detach().cpu(),
                    ),
                    dim=3,
                )
...
def dpm_solver(*, betas, denoiser, x_T, steps, cond, pre_ori):
    ...
    # Wrap to match: model(x, t_input, cond=...) -> noise | x_start
    def _model(x, t_input, cond):
        # DPM-Solver may pass float t_input; our UNet expects integer-ish step indices.
